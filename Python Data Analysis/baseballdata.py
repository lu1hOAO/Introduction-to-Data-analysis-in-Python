...
def filter_by_year(statistics, year, yearid):
    ...
    """
    result=list(filter(lambda item:item[yearid]==str(year),statistics))
    return result
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        cau_result=formula(info,row)
        tup=(row[info['playerid']],cau_result)
        result.append(tup)
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    result=[]
    string='{:.3f} --- {} {}'
    master_list=read_csv_as_list_dict(info['masterfile'], info['separator'], info['quote'])
    for player_id ,stat in top_ids_and_stats:
        for row in master_list:
            if row[info['playerid']]==player_id:
                text=string.format(stat,row[info['firstname']],row[info['lastname']])
                result.append(text)         
    return result
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    origin_statis=read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    year_statis=filter_by_year(origin_statis, year, info['yearid'])
    top_ids_and_stats=top_player_ids(info, year_statis, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    result={}
    for field in fields:
        for attemp in statistics:
            if attemp[playerid] not in result:
                result[attemp[playerid]]={}
                result[attemp[playerid]][playerid]=attemp[playerid]
            if field not in  result[attemp[playerid]]:
                result[attemp[playerid]][field]=0
           
            result[attemp[playerid]][field]+=int(attemp[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    player_lst=[]
    result=[]
    orin_statis=read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    player_dic=aggregate_by_player_id(orin_statis, info['playerid'], info['battingfields'])
    for value in player_dic.values():
        player_lst.append(value)
    top_ids_and_stats=top_player_ids(info, player_lst, formula, numplayers)
    result=lookup_player_names(info, top_ids_and_stats)
    return result
